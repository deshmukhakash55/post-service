...
public class SearchService {
    ...
    public List<DisplayPost> searchPostsByTag(String tag){
        ...
        for(PostMeta postMeta : postMetaList){
            ...
            DisplayPost post = new DisplayPost(postMeta.getId(), postMeta.getOwner(), Base64Utility.encode(postContent.getContent()), postMeta.getComments(), postMeta.getEmojis(), postMeta.getTags(), postMeta.getTagged_users(), postMeta.getCreation_time(), postMeta.getLatest_modified_time());
            posts.add(post);
        }
        return posts;
    }
    ...
    public List<DisplayPost> searchPostsBySearchedText(String searchText){
        ...
        for(PostMeta postMeta : postMetas){
            PostContent postContent = postContentRepository.findById(postMeta.getId()).get();
            if(BoyerMoore.indexOf(postContent.getContent(), searchText.getBytes()) != -1){
                DisplayPost post = new DisplayPost(postMeta.getId(), postMeta.getOwner(), Base64Utility.encode(postContent.getContent()), postMeta.getComments(), postMeta.getEmojis(), postMeta.getTags(), postMeta.getTagged_users(), postMeta.getCreation_time(), postMeta.getLatest_modified_time());
                posts.add(post);
            }
        }
